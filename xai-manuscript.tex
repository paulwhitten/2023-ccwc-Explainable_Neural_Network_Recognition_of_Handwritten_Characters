...
\begin{abstract}
    ...
    explainability.  Some methods, such as salience mapping and Gradient
    weighted Class Activation Mapping, provide a visual explanation by
    highlighting the portions of input that led to a decision.  This work
    introduces a partitioning approach to explainability and constructs an
    explainable data pipeline.  Decision making is distributed among explainable
    properties and associated neural networks to propose solutions.  The
    proposed property-based solutions are then combined to make a global
    decision and provide explainable rationale for the decision related to the
    properties that contributed to the solution.  The particular explainable
    pipeline implemented recognizes handwritten characters and produces
    explanations based on a character property analysis.  This work also
    approaches scalability by applying the explainable pipeline to a dataset
    with nearly five times more classes than prior work.  Increasing the accuracy
    of the pipeline via adding unexplainable solutions.  A metric is introduced
    that assists in characterizing explainability when an unexplainable
    component is added to the pipeline. A key challenge for supervised learning
    and explainability is ambiguous and mislabeled samples in training data.
    Techniques are applied to identify some of the issues with ambiguous and
    mislabeled data in EMNIST.  A concept explored in this work involves
    correctly pruning the training set in order to improve explainability and
    avoid misleading training set labels.  Samples from the training set that
    were pruned as label issues are presented back to the explainable pipeline
    in an attempt resolve issues and relabel them.  Results and explainable
    rationale from some pruned training set data is presented.
...
A property-based XAI data pipeline is introduced in Section~\ref{method} that
provides rationale for classification decisions, referencing explainable
...
properties, and ML models trained against those transformations are used to make
pose classification solutions.  The local, property-based, solutions are then
combined to make a global decision.  The properties used in the global decision
contribute rationale for the decision.  This methodology achieved acceptable
accuracy results with MNIST and was able to provide explanations related to
explainable properties of the samples.  Results from the explainable data
pipeline using MNIST are presented in Section~\ref{mnist_results}.

This work explores scaling issues as it applies the explainable data pipeline to
...
results of resolving some training issues using the explainable pipeline to
...
explainable data pipeline with the aim of recognizing handwritten characters while
...
label issues in a transparent way, providing an explanation for the resolution.
...
there appears to be a cost in decreased accuracy for explainability.
...
The quality of the training set is a big concern with NN and XAI.  The
...
This work introduced an XAI data pipeline for recognizing handwritten
characters.  This work increased the accuracy of the pipeline by adding highly
accurate but unexplainable components. A metric is introduced that assists in
characterizing explainability when unexplainable components is added to the
pipeline. The expansive EMNIST dataset was studied for explainable recognition
of characters and some challenges with training data were identified.  A key
concept to explainability that is introduced is correctly pruning the training
set in order to improve the explainability and avoid misleading training set
labels.  Results processing the EMNIST character database using the XAI data
pipeline are provided. The XAI pipeline was also used to resolve some of the
